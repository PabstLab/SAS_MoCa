...
#	1,2-dilauroyl-sn-glycero-3-phosphatidylglycerol
...
def Lipid_volume(T) :
	...
	return (1-x_PG) * (a0_V_DLPC +T * a1_V_DLPC) + x_PG * (a0_V_DLPG + T * a1_V_DLPG) 

#########################################################
def Volumes_DLPC(V_H_DLPC, T, r32, r_CG, r_PCN) :	
	"""Return volumes of each quasi-molecular group"""

	# Volumes 
	V_L = Lipid_volume(T)
	V_HC = V_L - ( (1-x_PG) * V_H_DLPC + x_PG * V_H_DLPG )

	# Quasi-molecular volumes 
	V_CH2	= V_HC / ( n_CH2 + n_CH3*r32 )		# Volume of CH2 groups  
	V_CH3	= V_CH2	*	r32 					# Volume of CH3 groups

	V_CG	= V_H_DLPC	*	r_CG				# Volume of CG group 
	V_PCN	= V_H_DLPC	* 	r_PCN 				# Volume of PCN group 
	V_Chol	= V_H_DLPC	*   (1 - r_PCN - r_CG)	# Volume of CholCH3 group  

	V_PG1	= V_H_DLPG	*	0.16 				# Pan et al. BBA 2012
	V_PG2	= V_H_DLPG	*	( 1 - 0.51 - 0.16) 	# Pan et al. BBA 2012
	
	return V_L, V_HC, V_CH2, V_CH3, V_CG, V_PCN, V_Chol, V_PG1, V_PG2

#########################################################
def SLDs_DLPC(T, xtris, xEDTA, V_Chol, V_PCN, V_CG, V_PG2, V_PG1, V_CH2, V_CH3, V_BW) :
...
##################
	def __init__(self, q, PAR, ompla_state) :
		...
		self.V_PG1, self.V_PG2] = Volumes_DLPC(self.V_H, self.T, self.r32, self.r_CG, self.r_PCN)
		...
		[rho_sol, self.drho_Chol, self.drho_PCN, self.drho_CG,
	 	...
	 	self.drho_BW] = SLDs_DLPC(	self.T, xtris, xEDTA, 
...
##################			
	def intensity(self):
		I = ( self.Norm * self.nv*1e5 ) * self.Ipoly
		return I + self.Con*( 0.99*(1./(1+np.exp(-8*(self.q-0.1)))) + 0.01 )	
...
##################			
	def SDP_profile(self):
		...
		n_W =  ( self.A_L * 2*self.d_BW/2.*erf(self.d_BW/2./(np.sqrt(2)*self.s_CH2)) - self.V_H ) / self.V_BW
		
		return 	np.column_stack((z_array, CH3, CH2, CG, PCN, Chol, BW, SLD)), n_W, 2*D_peak
